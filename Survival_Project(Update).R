...
# Lets look at the estimated survival curves of financial aid
attach(data.new)
data.fin <- with(data.new, data.frame(fin = c(0, 1), prio = rep(mean(prio), 2), employed = 1))
plot(survfit(fit.final.sub, newdata = data.fin), conf.int = F, lty = c(1, 2), ylim = c(0.6, 1), xlab = 'Weeks', ylab = 'Proportion not Rearrested')
